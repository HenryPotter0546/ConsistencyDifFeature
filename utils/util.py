...
def load_image_pair_for_test(ann, load_size, device, image_path="", output_size=None):
    img1_pil = Image.open(f"{image_path}/{ann['category']}/{ann['src_imname']}").convert("RGB")
    img2_pil = Image.open(f"{image_path}/{ann['category']}/{ann['trg_imname']}").convert("RGB")
    source_size = img1_pil.size
    target_size = img2_pil.size
    ann["src_imsize"] = source_size
    ann["trg_imsize"] = target_size

    # swap from (x, y) to (y, x)
    if "src_kps" in ann:
        source_points, target_points = ann["src_kps"], ann["trg_kps"]
        source_points = np.flip(source_points, 1)
        target_points = np.flip(target_points, 1)
        if output_size is not None:
            source_points = rescale_points(source_points, source_size, output_size)
