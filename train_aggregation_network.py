...
def log_aggregation_network(aggregation_network, config):
    ...
    if config["dual"] == True:
        negative_save_timestep = [-x for x in save_timestep]
        save_timestep = save_timestep + negative_save_timestep
        num_timesteps = len(save_timestep)
...
def get_hyperfeats(diffusion_extractor, aggregation_network, imgs):
    ...
    diffusion_hyperfeats = aggregation_network(feats.float().view((b, -1, w, h)).to("cuda:0"))
    ...
    #         img1_feats = diffusion_extractor.forward(imgs[0], prompt=" ")
    #         img2_feats = diffusion_extractor.forward(imgs[1], prompt=" ")
    #         b, s, l, w, h = img1_feats.shape
    # diffusion_hyperfeats = aggregation_network(img1_feats.float().view((b, -1, w, h)))
    # img1_hyperfeats = diffusion_hyperfeats[0][None, ...]
...
def main(args):
    ...
    val_anns = json.load(open(config["val_path"]))   #TODO
    if config.get("train_path"):         #TODO
